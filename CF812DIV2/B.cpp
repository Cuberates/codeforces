...
void solution(){
   ...
   if(b1 == a) cout << "YES \n";
      ...
      if(b1 == a) cout << "YES \n";
         ...
         // check for the third form increasing then decreasing
         int max_indice = 0;
         for(int i=0;i<n;i++) {
            if(a[i] >= a[max_indice]) max_indice = i;
         }
         int works = 1;
         for(int i=0;i<=max_indice-1 && works;i++) {
            if(a[i+1] < a[i]) works = 0;
         }
         for(int i=max_indice;i<n-1;i++) {
            if(a[i+1] > a[i]) works = 0;
         }
         if(works) cout << "YES \n";
         else cout << "NO \n";
